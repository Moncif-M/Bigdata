...
# Install Java 17 (fixes the cgroup v2 issue)
RUN apt-get update && \
    apt-get install -y software-properties-common && \
...
# set environment variables - UPDATED TO JAVA 17
...
ENV PATH=$PATH:/usr/local/hadoop/bin:/usr/local/hadoop/sbin:/usr/local/spark/bin:/usr/local/kafka/bin:/usr/local/hbase/bin:/usr/lib/jvm/java-17-openjdk-amd64/bin
